...
delayMicroseconds(uint32_t us)
{
	...
	const uint32_t tickPerMs = STK_RVR + 1;	/* XXX verify from platform_timing_init() or so */
...
SWIM_SET(void)
{
	...
	gpio_set(SWIM_IN_PORT, SWIM_IN_PIN);	/* activate pullup */
}
static void
SWIM_CLR(void)
{
...
SWIM_IN_TIMER_INIT(void)
{
	...
	TIM_SMCR(SWIM_IN_TIMER) |= TIM_SMCR_MSM;	/* from versaloon */
}

static void
SWIM_IN_TIMER_FINI(void) 
{
...
static void
SWIM_IN_TIMER_RISE_DMA_INIT(uint16_t len, uint32_t address)
{
...
static void
SWIM_OUT_TIMER_INIT(uint8_t channel, uint8_t polarity)
{
	rcc_periph_clock_enable(RCC_DMA1);
	...
	DMA1_CCR(channel) = 0;
	DMA1_IFCR |= DMA_IFCR_CIF(channel); // clear interrupts, == dma_clear_interrupt_flags(DMA1, channel, DMA_IFCR_CIF_BIT)
	dma_set_peripheral_address(DMA1, channel, (uint32_t)&TIM_CCR4(SWIM_OUT_TIMER));
	dma_set_read_from_memory(DMA1, channel);
	dma_set_number_of_data(DMA1, channel, 0); // filled in later
	dma_disable_peripheral_increment_mode(DMA1, channel);
	dma_enable_memory_increment_mode(DMA1, channel);
	dma_set_peripheral_size(DMA1, channel, DMA_CCR_PSIZE_16BIT); // check halfword
	dma_set_memory_size(DMA1, channel, DMA_CCR_MSIZE_16BIT);
	dma_set_priority(DMA1, channel, DMA_CCR_PL_MEDIUM);
...
static void
SWIM_OUT_TIMER_FINI(uint8_t channel)
{
...
static void
SWIM_OUT_TIMER_SetCycle (uint16_t cycle)
{
...
static void
SWIM_OUT_TIMER_DMA_INIT(uint8_t channel, uint16_t len, uint32_t address)
{
	TIM_EGR(SWIM_OUT_TIMER) = TIM_EGR_UG;
	DMA1_CCR(channel) &= ~1;
	DMA1_CNDTR(channel) = len;
	DMA1_CMAR(channel) = address;
	DMA1_CCR(channel) |= 1;
}
...
static void
SWIM_SetClockParam(uint8_t mHz, uint8_t cnt0, uint8_t cnt1)
{
	uint16_t clock_div;
...
static uint8_t
SWIM_HW_Out(uint8_t cmd, uint8_t bitlen, uint16_t retry_cnt)
{
...
retry:
	...
	*ptr++ = 0;
	...
	if (SWIM_DMA_IN_Buffer[bitlen + 2] > SWIM_PULSE_Threshold) {
		/* nack */
		if (retry_cnt) {
			retry_cnt--;
			goto retry;
...
static uint8_t
SWIM_HW_In(uint8_t* data, uint8_t bitlen)
{
	...
	*data = 0;
	if (dly && (SWIM_DMA_IN_Buffer[1] < SWIM_PULSE_Threshold)) {
		// sds : eerste bit moet 0 zijn, slave start
		for (dly = 0; dly < 8; dly++) {
 ...
 */
int
swim_setHighSpeed(bool highSpeed) /* F4-03 */
{
...
int
swim_doEntrySequence(void) /* F4-04 */
{
	uint32_t dly;
...
int
swim_readStatus(swimStatusAsync_t *status) /* F4-09 */
{
...
int
swim_wotf(uint32_t addr, uint16_t len, uint8_t *data) /* F4 - 0A */
{
	...
	if (swim_async) {
		if (len > SWIM_BUFFERSIZE)
			len = SWIM_BUFFERSIZE;
		/* an ongoing operation will be aborted; TODO CHECK is this according SWIM spec? */
	...
	if (len == 0 || data == NULL)	/* XXX ??? no BMP here */
		return ERR_FAIL;
...
int
swim_rotf(uint32_t addr, uint16_t len, uint8_t *data) /* F4 - 0B */
{
	uint16_t processed_len;
	uint8_t cur_len, i;
	uint32_t cur_addr, addr_tmp;
